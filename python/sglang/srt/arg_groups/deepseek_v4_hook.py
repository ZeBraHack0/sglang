...
def apply_deepseek_v4_defaults(server_args: ServerArgs, model_arch: str) -> None:
    ...
    if server_args.speculative_algorithm is not None:
        # The NSA paged pool has no move_kv_cache, so a tree draft (which accepts
        # non-contiguous slots) cannot run. EAGLE and linear-mode NGRAM both keep
        # accepts on the canonical contiguous slots.
