...
class ModelRunner(ModelRunnerKVCacheMixin):
    ...
    def _dummy_run(
        ...
        def get_spec_info():
            ...
            #     backend → EagleVerifyInput (the hybrid SUFFIX backend keeps
            #     EAGLE layout so the verify forward captures FULL hidden
            #     states for the MTP keep-up path)
            #   - short_chain runner: K = speculative_num_steps + 1 → MTP path
            #     → EagleVerifyInput
            #   - baseline runner:    K = 1 → no spec_info (plain decode)
            # Standalone SUFFIX is NOT affected: it is ngram-v2-native and
            # takes the NgramVerifyInput branch below.
            _hybrid_is_baseline = (
            ...
            elif self.spec_algorithm.is_ngram() and not _hybrid_is_baseline:
    ...
    def init_decode_cuda_graph(self):
        ...
        # (plain decode) path. The main `decode_cuda_graph_runner` above
        # captured K_suffix; instantiate the two narrower runners alongside it
        # and let `_forward_raw` dispatch by input-token width.
    ...
    def _forward_raw(
        ...
        with ctx_mgr:
            ...
            # CudaGraphRunner.can_run); the main `decode_cuda_graph_runner`
            # covers K_suffix. Try short-chain → baseline → main in
            # width-ascending order so the narrower (cheaper) graphs are
            # picked first.
            ...
            if mode_check():
                if short_chain_runner is not None and short_chain_runner.can_run(
                    ...
                    picked_runner = self.decode_cuda_graph_runner
            ...
            # runner from the dispatch above; == self.decode_cuda_graph_runner
            # for non-HYBRID algorithms).
            if can_run_graph:
