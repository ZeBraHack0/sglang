...
- ``_SuffixLike.is_ngram()`` returns ``True`` so SUFFIX transparently reuses
  ...
  source differs (see ``SuffixWorker``). NGRAM's worker is spec-v2 native, so
  SUFFIX inherits overlap-scheduling support (``supports_overlap=True``; the
  scheduler drives the same worker synchronously when overlap is disabled).
